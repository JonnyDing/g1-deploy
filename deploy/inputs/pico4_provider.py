...
from deploy.inputs.rot_utils import quat_mul_np
from deploy.interfaces import RealtimeInputProvider
from deploy.sim.reference_motion import interpolate_human_frames
...
class Pico4InputProvider(RealtimeInputProvider):
    ...
    def _normalize_pico_bridge_body_joints(body_joints: NDArray[np.float64]) -> NDArray[np.float64]:
        ...
        return np.asarray(body_joints, dtype=np.float64)
